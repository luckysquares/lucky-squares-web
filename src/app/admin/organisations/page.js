      ...
      {loading ? <div style={{ color: 'var(--text2)' }}>Loading…</div> : (
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {filtered.map((o) => (
              <div key={o.id} className="scratch-card" style={{ padding: '20px 24px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
                  <div style={{ flex: 1, minWidth: 240 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6, flexWrap: 'wrap' }}>
                      <span style={{ fontFamily: 'var(--font-serif)', fontSize: 16, fontWeight: 700 }}>{o.org_name}</span>
                      {isNew(o) && <span className="tag tag-green" style={{ fontSize: 11 }}>New</span>}
                      <span className={`tag ${o.status === 'approved' ? 'tag-green' : o.status === 'rejected' ? 'tag-muted' : ''}`} style={{ fontSize: 11, textTransform: 'capitalize' }}>{o.status ?? 'pending'}</span>
                    </div>
            ...
            {filtered.length === 0 && <div style={{ color: 'var(--text2)', fontSize: 14, padding: '40px 0', textAlign: 'center' }}>No applications found.</div>}
          </div>
        </>
      )}
    </div>
...
const DEMO_ORGS = [
  { id: '1', org_name: 'Sunbury Primary P&C', abn: '12 345 678 901', org_type: 'School P&C', contact_name: 'Mel Thompson', email: 'mel@sunburyprimary.edu.au', phone: '0412 345 678', suburb: 'Sunbury', state: 'VIC', postcode: '3429', status: 'pending',  created_at: new Date(Date.now() - 3 * 86400000).toISOString() },
  { id: '2', org_name: 'Wildlife Friends VIC', abn: '98 765 432 109', org_type: 'Registered charity', contact_name: 'Priya Sharma', email: 'priya@wildlife.org.au', phone: '0423 456 789', suburb: 'Geelong', state: 'VIC', postcode: '3220', status: 'approved', created_at: new Date(Date.now() - 18 * 86400000).toISOString() },
  { id: '3', org_name: 'Werribee Eagles AFC',  abn: '45 678 901 234', org_type: 'Sporting club', contact_name: 'Dave Kowalski', email: 'dave@werribeeeagles.com.au', phone: '0434 567 890', suburb: 'Werribee', state: 'VIC', postcode: '3030', status: 'pending',  created_at: new Date(Date.now() - 1 * 86400000).toISOString() },
];
